...
void _1path(int &x, int &y, int cost);
...
void path(int x, int y, vector<spot> &temp){
    ...
    while(cost > 1){
        cost--;
        if(x <= R_x){
            if( y <= R_y) _2path(x,y,cost);
            else _1path(x,y,cost);
        }
        else{
            if( y <= R_y) _3path(x,y,cost);
            else _4path(x,y,cost);
        }
        s.push(spot(x,y));
        map_clean[x][y] = true;
    }
...
{
    // 上右左下
    if( x-1>=0 && vis[x-1][y]==cost && !map_clean[x-1][y] ) x -= 1;
    else if( y+1<n && vis[x][y+1]==cost && !map_clean[x][y+1] ) y += 1;
    else if( y-1>=0 && vis[x][y-1]==cost && !map_clean[x][y-1] ) y -=1;
    else if( x+1<m && vis[x+1][y]==cost && !map_clean[x+1][y] ) x += 1;
